...
void TIM2_IRQHandler(void)
{
    ...
    {
        timer2_cur_time_ms++;
        TIM_ClearITPendingBit(TIM2, TIM_IT_Update);// —брасываем флаг прерывани€ (об€зательно!)
    }
}
...
void Tim2_count_mode_up(void)
{
	...
	// ¬ключаем прерывание по обновлению (Update Interrupt)
...
/*void Tim3_countdown_init_ms(void)
{
...
void Tim3_Delay_ms(uint32_t tim3_delay_new)
{
	timer3_cur_time_ms = tim3_delay_new;
	while(timer3_cur_time_ms) {}
}*/
...
void TIM3_IRQHandler(void)
{
    ...
    {
        tim3_10sec_flag = 1;
        TIM_ClearITPendingBit(TIM3, TIM_IT_Update);// —брасываем флаг прерывани€ (об€зательно!)
    }
}
